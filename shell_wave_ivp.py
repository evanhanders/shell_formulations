...
for formulation_index in run_formulations:
    ...
    if formulation_index in (3, 4, 5):
        ...
        problem = d3.IVP([ b, p, u, tau_b1, tau_b2, tau_u1, tau_u2], namespace=locals())

        problem.add_equation("dt(b) + dot(u, grad_b0) - kappa_ncc*div(grad_b) - dot(grad_b, grad(kappa_ncc)) + BC_b = 0")
        problem.add_equation("div_u = 0", condition="nθ != 0")
        problem.add_equation("dt(u) - b*er + grad(p) - nu*div(grad_u) + BC_u = 0", condition="nθ != 0")
        ...
        problem = d3.IVP([ b, p, u, tau_p, tau_b1, tau_b2, tau_u1, tau_u2], namespace=locals())

        problem.add_equation("dt(b) + dot(u, grad_b0) - kappa_ncc*div(grad_b) - dot(grad_b, grad(kappa_ncc)) + BC_b = 0")
        problem.add_equation("div_u + tau_p = 0")
        problem.add_equation("dt(u) - b*er + grad(p) - nu*div(grad_u) + BC_u = 0")
    ...
    # Solver
    solver = problem.build_solver(d3.SBDF2, ncc_cutoff=ncc_cutoff)
    solver.stop_iteration = 50001
    ...
    b.fill_random(layout='g', seed=42, distribution='normal', scale=1e0)
    b['g'] *= np.sin(θ)
    b['g'] *= np.cos(np.pi*(r - Ri)/(Ro - Ri))

    while solver.proceed:
        if (solver.iteration - 1) % 100 in [0, 1]:
            for f in solver.state:
                f.require_grid_space()
        solver.step(timestep)
        if (solver.iteration - 1) % 1000 == 0:
            max_u = np.max(u['g'])
            this_str = "iter = {:06d}, max_u = {:.4e}".format(solver.iteration, max_u)
